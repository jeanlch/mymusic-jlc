...
class Tracks {
	...
	async all() {
		...
		for (const index in tracks) {
			if(tracks[index].art === null) tracks[index].art =
                'https://orlando-road.codio-box.uk/public/data/testimage.jpg'
			const seconds = tracks[index].duration
			const minnumber = 60
			const secnumber = 3600
			const min = Math.floor(seconds % secnumber / minnumber)
			const sec = Math.floor(seconds % secnumber % minnumber)
